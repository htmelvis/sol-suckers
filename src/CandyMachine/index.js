...
const CandyMachine = ({ walletAddress }) => {
  ...
  const [goLiveDateTimeString, setGoLiveDateTimeString] = useState();
  ...
  const getCandyMachineState = async () => {
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        if (!mints.find((mint) => mint === parse.image)) {
          setMintData((prevState) => [...prevState, parse]);
    ...
    setGoLiveDateTimeString(goLiveDateTimeString);
  };
  ...
  const renderDropTimer = () => {
    ...
    return <p>{`Drop Date: ${goLiveDateTimeString}`}</p>;
  };

  return (
    <>
      {isLoadingMints && <p className="loading">LOADING MINTS...</p>}

      {itemsAvailable && (
        <div className="machine-container">
          {renderDropTimer()}
          <div className="sub-text gradient-text">
            {itemsRedeemed === itemsAvailable ? (
              <p>
                <span>Sorry! We're sold out!</span>
              </p>
            ) : (
              <>
                <p>
                  <span>Mint your sol-sucking ghost now!</span>
                </p>
                <p>
                  Items Minted: {itemsRedeemed} out of {itemsAvailable} only{" "}
                  {itemsRemaining} left.
                </p>
          ...
          {mintData.length > 0 && renderMintedItems()}
        </div>
      )}
    </>
  );
